...
class AdafactorOptimizer(tf.keras.optimizers.Optimizer):
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        if coefficients["clipping_threshold"] is not None:
            clipping_denom = tf.maximum(
                1.0, reduce_rms(x) / coefficients["clipping_threshold"]
            )
            x /= clipping_denom
        ...
        if self.has_beta_1:
            ...
            new_m = (
                coefficients["beta1"] * tf.cast(m, tf.float32)
    ...
    def _cast_like(self, x, y):
        ...
        Grabbed from tensor2tensor/layers/common_layers
        """
    ...
    def _adafactor_decay_rate_pow(self, exponent):
        """Second moment decay rate where memory-length grows as
        step_num^exponent.
...
class AdamWeightDecay(tf.keras.optimizers.Adam):
    ...
    will interact with the m and v parameters in strange ways. Instead we want
    ot decay the weights in a manner that doesn't interact with the m/v
    parameters. This is equivalent to adding the square of the weights to the
    loss with plain (non-momentum) SGD.
    """
